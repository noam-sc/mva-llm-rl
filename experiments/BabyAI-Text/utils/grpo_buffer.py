'''
Advantage calculation changed based on GRPO approach.
...
class PPOBuffer:
    ...
    def __init__(self, size, gamma=0.99, lam=0.95):
        ...
        self.rew_mean = np.zeros(size, dtype=np.float32)#
        self.rew_std  = np.zeros(size, dtype=np.float32)#
        self.gamma, self.lam = gamma, lam
        self.ptr, self.path_start_idx, self.max_size = 0, 0, size

    def store(self, obs, possible_act, act, rew, val, logp, rew_mean, rew_std):
        """
        ...
        self.rew_mean[self.ptr] = rew_mean
        self.rew_std[self.ptr] = rew_std
        self.ptr += 1
    ...
    def finish_path(self, last_val=0):
        ...
        self.adv_buf[path_slice] = (self.rew_buf[path_slice] - self.rew_mean[path_slice])\
                                    /(self.rew_std[path_slice] + 0.00001)
